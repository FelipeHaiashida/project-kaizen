...
- **Notificações (S15):** `@/lib/notifications` `notify()` (respeita `NotificationSettings`, pula self). `NotificationBell` no header (realtime na tabela `Notification`). `/settings/notifications`.
